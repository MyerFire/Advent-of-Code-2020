...
class Passport:
    ...
    def check_part_two(self):
        ...
        if not self.hcl.startswith("#") or (len(re.findall(r"[a-f0-9]", self.hcl)) != 6): return False
...
def join_(list_):  # join the lines of the file together by each passport entry
    joined = []
    for entry in list_:
        if not bool(joined) or entry == "":  # not bool(joined) only pertains to the very first line
