...
def parse(data):
    row_specification = data[:7]
    column_specification = data[-3:]
    ...
    for character in column_specification:
        if character == "L":
            ...
            column = column[len(column) // 2:]
    id_ = row[0] * 8 + column[0]
    return row[0], column[0], id_


def sort_key(boarding_pass):
...
if __name__ == "__main__":
    ...
    for index, boarding_pass in enumerate(boarding_passes):
        if index == 0:
            ...
            if boarding_pass.id + 1 != boarding_passes[index - 1].id:
                print(f"There is a missing seat between seat ID {boarding_pass.id} and {boarding_passes[index - 1].id}"
                      f", which is {(boarding_pass.id + boarding_passes[index - 1].id) // 2}")
